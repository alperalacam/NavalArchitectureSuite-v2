...
│   └── SurfaceAnalysis.cs           ← Zebra / porcupine curvature analysis
│
├── Modules\                         ← Calculation modules (linked to Toolkit volumes)
│   ├── Hydrostatics\                ← Vol 1, 29, 30 — exact from OCCT surface
│   ├── Stability\                   ← Vol 29, 30 — GZ, loading, free surface
│   ├── DamageStability\             ← Vol 8, 26, 30 — probabilistic, SOLAS
│   ├── Resistance\                  ← Vol 3, 12, 39 — Holtrop / Savitsky / Insel-Molland
│   ├── Propulsion\                  ← Vol 3, 39, 41 — B-series, waterjet, electric
│   ├── Manoeuvring\                 ← Vol 5, 27, 31 — rudder, turning circle, DP
│   ├── Seakeeping\                  ← Vol 2, 13 — RAO, spectrum, Nordforsk
│   ├── Structures\                  ← Vol 4, 9, 40 — scantlings, FEM, composites
│   ├── Thermal\                     ← Vol 35 — heat transfer FEM, HVAC, exchangers
│   ├── Machinery\                   ← Vol 18, 19, 20, 36 — engines, pumps, compressors
│   ├── Electrical\                  ← Vol 37, 38 — IEC 60092, PMS, DP power
│   ├── Submarine\                   ← Vol 6 — pressure hull, reserve buoyancy
│   ├── Offshore\                    ← Vol 14 — FPSO, semi-sub, mooring, API loads
│   ├── UnderwaterNoise\             ← Vol 33 — radiated noise, cavitation
│   ├── IceClass\                    ← Vol 34 — Lindqvist, PC loads, Polar Code
│   ├── LNG\                         ← Vol 28 — BOG, FGSS, IGC Code
│   ├── Composites\                  ← Vol 40 — laminate theory, Tsai-Wu, sandwich
│   └── ClassRules\                  ← RINA / LR / ABS / Turkish Lloyd engine
│
├── VesselIntelligence\              ← Category engine — valid formula selection per vessel
│   ├── VesselCategory.cs            ← 16 categories, 195 vessel types
│   ├── FormulaApplicability.cs      ← Which methods apply to which category
│   └── ParameterRanges.cs           ← Valid L/B, L/T, Cb ranges per category
│
├── FormulaTrace\                    ← Transparent calculation audit system
│   ├── TraceEngine.cs               ← Records every step of every calculation
│   ├── TracePanel.xaml              ← Click-to-inspect UI panel
│   └── SourceCitations.cs           ← Links to Vol, page, equation number
│
├── EngineDatabase\                  ← Main engines, auxiliaries, pumps (200+ entries)
│   ├── MainEngines.db               ← MAN B&W, WinGD, Wärtsilä, Bergen, MTU...
│   ├── AuxiliaryGenerators.db       ← Caterpillar, Cummins, Wärtsilä gensets
│   └── Pumps.db                     ← Ballast, bilge, fire, LO, FO, cooling
│
├── LinesPlan\                       ← Body plan, sheer plan, half-breadth
│
├── LicenseManager\                  ← Online activation, seat management
│   ├── LicenseClient.cs             ← 7-day heartbeat, AES-256 local cache
│   └── ActivationDialog.xaml        ← License key entry UI
│
